...
#### Note: For questions 4-5 inclusive, please refer to the other PDF containing Q7/our appendix for all the necessary code, graphs, statistical analyses. 
...
```{r, echo = FALSE, fig.height = 5, fig.width = 8, include= F}
...
```
```{r, echo = F, fig.height = 5, fig.width = 8, include = F}
# NAs grouped by education
p1a <- dat[!complete.cases(dat),] %>%  # keep rows with NAs
    pivot_longer(colnames(ed)[8:11], names_to = "Education", values_to = "Values") %>% # pivot into long table
...
```{r, fig.height = 5, fig.width = 8, echo = F, include= F}
# NAs grouped by education
p1a <- dat[!complete.cases(dat),] %>%  # keep rows with NAs
    pivot_longer(colnames(ed)[8:11], names_to = "Education", values_to = "Values") %>% # pivot into long table
...
From our EDA, we explored how much missing data there is:
...
Based on the summary function of mod1.4 (refer to Q7 appendix), we see that the adjusted R-squared value of model 1.4 is higher than model 1.3 and it is beneficial to include a fuller description of the racial distribution. However, we can also see that the Asian and Black percentages are not very significant. By definition, there should be some interactions between these percentages, and it is worth it to check it out. 

We see that the value of PctBlack and PctWhite are strongly negatively correlated (with value -0.8312116), and the other two percentages are weakly correlated (with values -0.2658648 for PctAsian vs. PctWhite, and -0.2331931 for PctOtherRace and PctWhite). Thus, we should remove PctBlack from the model to reduce colinearity. 

Based on the summary function of mod1.5 (model with PctBlack removed), after removing the PctBlack covariate, we see that there is a significant increase in adjusted R-squared value to mod1.1(baseline) and LRT results suggest the models are significantly different from each other with p value < 2.2e-16 (reject null hypothesis). Let's take another look at the median income covariates again. My main interest here is that whether a quadratic term would benefit our model, as a income could have diminishing return effect on lung cancer prevention/death rate. 
...
Judging from summary output of mod1.6.1 and mod1.6, the quadratic model performs better than the purely linear model (based on adjusted R-squared value) and the two models are statistically different (the LRT test resulted in p < 2.2e-16, so we reject the null hypothesis). However, a visual inspection of the data doesn't really show that the quadratic model is significantly better. At this point, we decide to keep the quadratic term due to the diagnostic stats. 

We now have this following core model: mod1_core = lm(data = dat, TARGET_deathRate ~ medIncome + I(medIncome ^2)+ PctWhite + PctAsian + PctOtherRace)
```{r, echo = F, include=F}
...
The residual diagnostic was evaluated to confirm the model's validity. 
...
Please refer to the chunk of code under the "Model fits" subtitle in the Q7 appendix. Here we find that the negative binomial regression has slightly higher standard errors and resulting test statistics compared to the standard Poisson regression. This is as we expected since it suggests that the standard Poisson regression may cause overconfidence in model evaluations, due to the property of $\mathbb{E}[Y] = Var(Y)$ being unsatisfied.
